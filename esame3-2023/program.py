...
def func1(D):
    return [sorted(value, reverse=True)[key] for key, value in D.items()]
...
def func2(list_all, list_rm):
    list_all[:] = (i for i in list(set(list_all)) if i in list_rm)
    return list_all
...
def func5(points):
    cpoints = sorted(points, key= lambda x: round((x[0]**2 + x[1]**2)**(1/2), 3))[:3]
    Xp = round((cpoints[0][0] + cpoints[1][0] + cpoints[2][0]) / 3, 3)
    Yp = round((cpoints[0][1] + cpoints[1][1] + cpoints[2][1]) / 3, 3)
    return Xp,Yp
