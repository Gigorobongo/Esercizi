...
def func1(string_list1, string_list2):
    ...
    for i in string_list2:
        if i not in string_list1:
            final_string.append(i)
    return sorted(final_string, reverse=True)  
...
def func3(a_list):
    ...
    starting_len = len(a_list)
    maxi = max(a_list)
    mini = min(a_list)
    
    a_list[:] = (i for i in a_list if i!=maxi and i!=mini)
    
    return starting_len - len(a_list)
...
def ex1(directory, ext):
    ...
    for address in listdir:
        path = directory + '/' + address
        if os.path.isdir(path):
            res.update(ex1(path, ext))
        elif os.path.isfile(path) and path.endswith(ext):
            with open(path) as r:
                text = r.read()
                if directory not in res:
                    res[directory] = len(text)
                else:
                    res[directory] += len(text)
    return res
...
def ex2(root):
    res = {}
    final = 0
    for key,value in ex2a(root, 0, res).items():
        if key % 2 == 0:
            final += value
        else:
            final -= value
